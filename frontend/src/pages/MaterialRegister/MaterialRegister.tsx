...
import { getMaterialList } from '../../store/basic/materialReducer';
...
const MaterialRegister: React.FC = () => {
  ...
  const data = useAppSelector((state) => state.material.materialList);
  ...
  const [modalData, setModalData] = useState<MaterialInterface>({
    id: 0,
    name: "",
    code: ""
  });
  ...
  const DeleteModal = (
    <Dialog open={deleteOpen}>
      <DialogTitle textAlign="center">素材削除</DialogTitle>
      <DialogContent>
        削除しましょか？
      </DialogContent>
      <DialogActions sx={{ p: '1.25rem' }}>
