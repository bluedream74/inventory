...
import {
  CollectionSlipInterface,
  getCollectionSlipList,
} from "../../store/slip/collectionSlipReducer";
...
export const CollectionSlip = () => {
  ...
  const [selectedSlip, setSelectedSlip] = useState<CollectionSlipInterface>({
    id: 0,
    no: "",
  ...
  const collectionList = useAppSelector((state) => state.collectionSlip.slips);
  ...
  const handleSave = () => {
    ...
    if (selectedSlip.no === "新規登録") {
      ...
      axiosApi
        .put(`slip/collection_slip/${slip_id}`, selectedSlip)
        .then((res) => {
          console.log(res);
          dispatch(getCollectionSlipList());
        })
  ...
  const handleNoChange = (noValue: string | null) => {
    if (noValue && noValue !== "新規登録") {
      ...
      setSelectedSlip(collectionList[index]);
      setRows(collectionList[index]?.items);
      setBtnStatus({
        ...btnStatus,
        first:
