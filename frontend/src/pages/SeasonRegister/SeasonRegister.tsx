...
import { getSeasonList } from '../../store/basic/seasonReducer';
...
const SeasonRegister: React.FC = () => {
  const dispatch = useAppDispatch();
  const data = useAppSelector((state) => state.season.seasonList);
  ...
  const [modalData, setModalData] = useState<SeasonInterface>({
    id: 0,
    name: "",
    code: ""
  });
  ...
  const DeleteModal = (
    <Dialog open={deleteOpen}>
      <DialogTitle textAlign="center">シーズン削除</DialogTitle>
      <DialogContent>
        削除しましょか？
      </DialogContent>
      <DialogActions sx={{ p: '1.25rem' }}>
