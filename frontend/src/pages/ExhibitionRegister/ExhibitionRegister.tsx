...
import './ExhibitionRegister.scss';
...
const ExhibitionRegister: React.FC = () => {
  const dispatch = useAppDispatch();
  const data = useAppSelector((state) => state.exhibition.exhibitionList);
  ...
  const [modalData, setModalData] = useState<ExhibitionInterface>({
    id: 0,
    name: "",
    code: ""
  });
  ...
  const DeleteModal = (
    <Dialog open={deleteOpen}>
      <DialogTitle textAlign="center">展示会削除</DialogTitle>
      <DialogContent>
        削除しましょか？
      </DialogContent>
      <DialogActions sx={{ p: '1.25rem' }}>
