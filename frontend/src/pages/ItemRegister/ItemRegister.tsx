...
import { getItemList } from '../../store/basic/itemReducer';
...
const ItemRegister: React.FC = () => {
  ...
  const data = useAppSelector((state) => state.item.itemList);
  ...
  const [modalData, setModalData] = useState<ItemInterface>({
    id: 0,
    name: "",
    code: ""
  });
  ...
  const DeleteModal = (
    <Dialog open={deleteOpen}>
      <DialogTitle textAlign="center">アイテム削除</DialogTitle>
      <DialogContent>
        削除しましょか？
      </DialogContent>
      <DialogActions sx={{ p: '1.25rem' }}>
